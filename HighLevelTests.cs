...
namespace Com.MarcusTS.SmartDI.MSTests
{
   ...
   [TestClass]
   public class HighLevelTests : IDisposable
   {
      ...
      public void TestArguments()
      {
         ...
         // 2. SetThrowOnAttemptToAssignDuplicateContractSubType to true, which should throw an error under the same scenario that just succeeded above.
      ...
      public void TestComplexConstructorsAndRecursion()
      {
         ...
         AssertContainerHasThrownOperationException(
      ...
      {
         _container.RegisterType<ClassWithOptionalParameters>();

         // Register one required types.  This will fail because all required parameters are indeed mandatory.
         _container.RegisterType<RequiredParam1>();
         var classWithOptionalParameters = _container.Resolve<ClassWithOptionalParameters>();

         AssertContainerHasThrownOperationException("Was allowed to resolve with a missing required parameter.");
         
         // Clear the slate
         _container.ClearUnitTestExceptions();

         // Add the second required parameter.
         _container.RegisterType<RequiredParam2>();
         classWithOptionalParameters = _container.Resolve<ClassWithOptionalParameters>();

         // Should resolve
         AssertContainerHasRaisedNoExceptions();

         // Inject an optional parameter
         _container.RegisterType<OptionalParam1>();
         classWithOptionalParameters = classWithOptionalParameters = _container.Resolve<ClassWithOptionalParameters>();

         // Should resolve and be injected
         AssertContainerHasRaisedNoExceptions();
         Assert.IsTrue( classWithOptionalParameters.InjectedOptionalParam1 != null && classWithOptionalParameters.InjectedOptionalParam2 == null, "Injected one optional parameter but did not get it back." );

         // Inject the second optional parameter
         _container.RegisterType<OptionalParam2>();
         classWithOptionalParameters = classWithOptionalParameters = _container.Resolve<ClassWithOptionalParameters>();

         // Should resolve and be injected
         AssertContainerHasRaisedNoExceptions();
         Assert.IsTrue( classWithOptionalParameters.InjectedOptionalParam1 != null && classWithOptionalParameters.InjectedOptionalParam2 != null, "Injected both optional parameters but did not get both back." );
      }
